...
**hmm:** Whether the unit hypercircle dimension (n) is fixed or parametric — the traversal depth is resolved as fixed (≤3), but the per-disk cross-section dimensionality remains open. See `ucns-embedding-correction.md` in erinepshovel-code/unitcircle.
